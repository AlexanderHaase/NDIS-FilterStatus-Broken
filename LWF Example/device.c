...
#pragma NDIS_INIT_FUNCTION(LWFExampleRegisterDevice)


_IRQL_requires_max_(PASSIVE_LEVEL)
NDIS_STATUS
LWFExampleRegisterDevice(
    VOID
    )
{
    ...
    DEBUGP(DL_TRACE, "==>LWFExampleRegisterDevice\n");
   
    
    NdisZeroMemory(DispatchTable, (IRP_MJ_MAXIMUM_FUNCTION+1) * sizeof(PDRIVER_DISPATCH));
    
    DispatchTable[IRP_MJ_CREATE] = LWFExampleDispatch;
    DispatchTable[IRP_MJ_CLEANUP] = LWFExampleDispatch;
    DispatchTable[IRP_MJ_CLOSE] = LWFExampleDispatch;
    DispatchTable[IRP_MJ_DEVICE_CONTROL] = LWFExampleDeviceIoControl;
    ...
    DEBUGP(DL_TRACE, "<==LWFExampleRegisterDevice: %x\n", Status);
